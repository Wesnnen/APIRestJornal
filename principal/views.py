...
class PublicacaoDetalheApiView(APIView):
    ...
    def retornarErro(self):
        return Response({"resposta":"A Publicação não foi encontrada"}, status=status.HTTP_404_NOT_FOUND)
    ...
    def delete(self, request, id, *args, **kwargs):
        ...
        if publicacao is not None:  
            publicacao.delete()
            return Response({"resposta": "A publicaçao foi excluida com sucesso."}, status=status.HTTP_200_OK)
        else:
            return self.retornarErro()
